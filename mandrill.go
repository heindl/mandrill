...
func (m *Client) RenderTemplate(message *Message, templateName string) (string, error) {

	data := map[string]interface{}{
		"key": m.Key,
		"template_name": templateName,
		"template_content": message.GlobalMergeVars, // []*Variable
	}
	...
	url := fmt.Sprintf("%stemplates/render.js", m.BaseURL)
	...
	if resp.StatusCode >= 400 {
		...
		if err := json.Unmarshal(body, apiError); err != nil {
			return "", errors.New(fmt.Sprintf("Could not unmarshal Mandrill API error[%d]: %s", resp.StatusCode, err))
		}
		return "", errors.New(fmt.Sprintf("Mandrill API Error[%i]: %s", resp.StatusCode, apiError))
	}
